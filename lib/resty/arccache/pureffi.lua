...
function _M.set(self, key, value, ttl, flags)
    ...
    if self.b1:count() > self.size-self.p then
        self.b1:delete_oldest()
    end

    if self.b2:count() > self.p then
        self.b2:delete_oldest()
    end
